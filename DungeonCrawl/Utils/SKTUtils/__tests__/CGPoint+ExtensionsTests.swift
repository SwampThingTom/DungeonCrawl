...
class CGPointExtensionsTests: XCTestCase {
    ...
    func testInitWith45DegreeAngle() {
        let a = π/4.0
        let pt = CGPoint(angle: a)
        XCTAssertEqual(pt.x, 1.0/sqrt(2.0), accuracy: CGFloat.ulpOfOne)
        XCTAssertEqual(pt.y, 1.0/sqrt(2.0), accuracy: CGFloat.ulpOfOne)
    }
    
    func testInitWith90DegreeAngle() {
        let a = π/2.0
        let pt = CGPoint(angle: a)
        XCTAssertEqual(pt.x, CGFloat(0.0), accuracy: CGFloat.ulpOfOne)
    ...
    func testInitWith180DegreeAngle() {
        ...
        XCTAssertEqual(pt.y, CGFloat(0.0), accuracy: CGFloat.ulpOfOne)
    }
    ...
    func test180DegreeAngle() {
        let pt = CGPoint(x: -1.0, y: 0.0)
        XCTAssertEqual(pt.angle, π, accuracy: 1.0e-6)
    }
    
    func testMinus135DegreeAngle() {
        let pt = CGPoint(x: -1.0/sqrt(2.0), y: -1.0/sqrt(2.0))
        XCTAssertEqual(pt.angle, -3.0*π/4.0, accuracy: CGFloat.ulpOfOne)
    }
    ...
    func testDistance() {
        XCTAssertEqual(pt1.distanceTo(pt2), CGFloat(100.62305898749054), accuracy: CGFloat.ulpOfOne)
    }
    
    func testThatLengthEqualsDistance() {
        XCTAssertEqual(pt1.distanceTo(pt2), (pt1 - pt2).length(), accuracy: CGFloat.ulpOfOne)
    }
    
    func testNormalized() {
        let normalized = pt1.normalized()
        XCTAssertEqual(normalized.x, 2.0/sqrt(5.0), accuracy: CGFloat.ulpOfOne)
        XCTAssertEqual(normalized.y, 1.0/sqrt(5.0), accuracy: CGFloat.ulpOfOne)
    }
    ...
    func testThatNormalizeReturnsNewValue() {
        _ = pt1.normalize()
        XCTAssertEqual(pt1.x, 2.0/sqrt(5.0), accuracy: CGFloat.ulpOfOne)
        XCTAssertEqual(pt1.y, 1.0/sqrt(5.0), accuracy: CGFloat.ulpOfOne)
    }
    ...
    func testLerp() {
        ...
        for t in stride(from: 0.0, through: 1.0, by: 0.1) {
            ...
            print("\(i), \(t)")
            XCTAssertEqual(lerped.x, expected[i].x, accuracy: 1.0e6)
            XCTAssertEqual(lerped.y, expected[i].y, accuracy: 1.0e6)
            i += 1
        }
    }
}
