...
class CGVectorTests: XCTestCase {
    ...
    func testInitWith45DegreeAngle() {
        let a = π/4.0
        let v = CGVector(angle: a)
        XCTAssertEqual(v.dx, 1.0/sqrt(2.0), accuracy: CGFloat.ulpOfOne)
        XCTAssertEqual(v.dy, 1.0/sqrt(2.0), accuracy: CGFloat.ulpOfOne)
    }
    
    func testInitWith90DegreeAngle() {
        let a = π/2.0
        let v = CGVector(angle: a)
        XCTAssertEqual(v.dx, CGFloat(0.0), accuracy: CGFloat.ulpOfOne)
    ...
    func testInitWith180DegreeAngle() {
        ...
        XCTAssertEqual(v.dy, CGFloat(0.0), accuracy: CGFloat.ulpOfOne)
    }
    ...
    func test180DegreeAngle() {
        let v = CGVector(dx: -1.0, dy: 0.0)
        XCTAssertEqual(v.angle, π, accuracy: 1.0e-6)
    }
    
    func testMinus135DegreeAngle() {
        let v = CGVector(dx: -1.0/sqrt(2.0), dy: -1.0/sqrt(2.0))
        XCTAssertEqual(v.angle, -3.0*π/4.0, accuracy: CGFloat.ulpOfOne)
    }
    ...
    func testDistance() {
        XCTAssertEqual(v1.distanceTo(v2), CGFloat(100.62305898749054), accuracy: CGFloat.ulpOfOne)
    }
    
    func testThatLengthEqualsDistance() {
        XCTAssertEqual(v1.distanceTo(v2), (v1 - v2).length(), accuracy: CGFloat.ulpOfOne)
    }
    
    func testNormalized() {
        let normalized = v1.normalized()
        XCTAssertEqual(normalized.dx, 2.0/sqrt(5.0), accuracy: CGFloat.ulpOfOne)
        XCTAssertEqual(normalized.dy, 1.0/sqrt(5.0), accuracy: CGFloat.ulpOfOne)
    }
    ...
    func testThatNormalizeReturnsNewValue() {
        _ = v1.normalize()
        XCTAssertEqual(v1.dx, 2.0/sqrt(5.0), accuracy: CGFloat.ulpOfOne)
        XCTAssertEqual(v1.dy, 1.0/sqrt(5.0), accuracy: CGFloat.ulpOfOne)
    }
    ...
    func testLerp() {
        ...
        for t in stride(from: 0.0, through: 1.0, by: 0.1) {
            ...
            XCTAssertEqual(lerped.dx, expected[i].dx, accuracy: 1.0e6)
            XCTAssertEqual(lerped.dy, expected[i].dy, accuracy: 1.0e6)
            i += 1
        }
    }
}
